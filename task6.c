...
static uintptr_t run_trial(struct gate_memory *memory, int input_cached,
                           uintptr_t trash,
...
static void record_result(struct accuracy_results *results, int input_cached,
...
int main(void)
{
    ...
    for (int trial = 0; trial < WARMUP_TRIALS; ++trial) {
        struct observed_outputs outputs;
        trash = run_trial(&memory, rand() % 2, trash, &outputs);
    }
    ...
    for (int trial = 0; trial < TRIALS; ++trial) {
        ...
        trash = run_trial(&memory, input_cached, trash, &outputs);
        record_result(&results, input_cached, &outputs);
    }

    printf("FAN2 gate:\n");
    report_accuracy(&results, 0);
    report_accuracy(&results, 1);
